...
public class OAPLogout extends TestBase {
	...
	@Test
	public void OAPverifyLogOut(Method method)

	{
		try {
			Log.startTestCase("OAPverifyLogOut");
			snbTestCases = snbreports.startTest("SNBUSA-56: OAP User LogOut");
			snbTestCases.log(LogStatus.INFO, "Opening Google Page","Start test");
			Assert.assertEquals(driver.getCurrentUrl(),"http://www.google.co.in");
			snbTestCases.log(LogStatus.INFO, "OAP Landing Page is displayed", snbTestCases.addScreenCapture(ExtentManager.CaptureScreen(driver, Constants.OAPscreenShot_Path+method.getName())));
			snbTestCases.log(LogStatus.PASS,"Google Page Opened","Test Case Passed");
